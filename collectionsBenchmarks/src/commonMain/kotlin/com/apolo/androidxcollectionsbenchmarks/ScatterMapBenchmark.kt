...
public sealed class ScatterMap<K, V> {
    ...
    @PublishedApi internal var metadata: LongArray = EmptyGroup

    @PublishedApi internal var keys: Array<Any?> = EMPTY_OBJECTS

    @PublishedApi internal var values: Array<Any?> = EMPTY_OBJECTS
...
public class MutableScatterMap<K, V>(initialCapacity: Int = DefaultScatterCapacity) :
    ...
    private fun initializeMetadata(capacity: Int) {
        metadata =
            if (capacity == 0) {
                ...
                la.fill(AllEmpty)
                writeRawMetadata(la, capacity, Sentinel)
                la
            }
        initializeGrowth()
    }
    ...
    public operator fun set(key: K, value: V) {
        val insertIndex = findInsertIndex(key)
        val index = if (insertIndex < 0) insertIndex.inv() else insertIndex
        keys[index] = key
        values[index] = value
    }
...
internal inline fun hash(k: Any?): Int {
    ...
    val hashCode = if (k != null) k.hashCode() else 0
    val hash = hashCode * MurmurHashC1
